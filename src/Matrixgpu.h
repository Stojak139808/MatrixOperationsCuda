...
__global__ void subtractKernel(const float* A, const float* B, float* Y, const int rows, const int columns) {
...
public:
    ...
    Matrixgpu operator - (const Matrixgpu& x) {
        ...
        if (this->columns == x.columns && this->rows == x.rows) {
            ...
            dimGrid.y = (this->rows + dimBlock.y - 1) / dimBlock.y;
            dimGrid.z = 1;
            subtractKernel <<<dimGrid, dimBlock >>> (this->dev_matrix, x.dev_matrix, y.dev_matrix, this->rows, this->columns);
